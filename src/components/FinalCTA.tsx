...
const FinalCTA = ({
  ...
  t: { title: string; subtitle: string; button: string; href: string };
}) => {
  // Decide whether to use Link or <a>
  const isInternal =
    t.href.startsWith("/") && !t.href.startsWith("//") && !t.href.startsWith("http");

  const ButtonElement = isInternal
    ? (
      <Link
        to={t.href}
