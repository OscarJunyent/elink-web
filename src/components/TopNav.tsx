...
const TopNav = () => {
  ...
  return <nav className="w-full flex items-center justify-between py-2 px-3 bg-white/95 backdrop-blur border-b border-gray-200 sticky top-0 z-40 shadow-circular">
      ...
      <div className="flex md:hidden">
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger asChild>
            <button aria-label="Obrir el menú" className="p-2 rounded-md text-[#222c38] hover:bg-[#f7d7db] focus-visible:ring-2 focus-visible:ring-primary transition-all" onClick={() => setOpen(true)}>
              <Menu size={28} />
            </button>
          </SheetTrigger>
            ...
            <nav>
              <ul className="flex flex-col gap-1 px-4">
                ...
                return <li key={item.to}>
                      <Link to={item.to} onClick={handleLinkClick} className={`block w-full text-lg px-4 py-3 rounded font-medium transition-all duration-150 ${active ? "bg-[#c92637] text-white shadow" : "text-[#222c38] hover:bg-[#f7d7db] hover:text-[#c92637]"}`} style={{
                ...
                <li key="blog">
                  <a href={blogMenu.href} target="_blank" rel="noopener noreferrer" onClick={handleLinkClick} className={`block w-full text-lg px-4 py-3 rounded font-medium transition-all duration-150 text-[#222c38] hover:bg-[#f7d7db] hover:text-[#c92637]`} style={{
                  fontFamily: "Montserrat, Inter, sans-serif"
                }}>
                    {blogMenu.label}
                  </a>
      ...
      <ul className="hidden md:flex gap-1 md:gap-3 xl:gap-8 items-center font-sans select-none">
        ...
        {/* Blog last item as external link */}
        <li key="blog">
          <a href={blogMenu.href} target="_blank" rel="noopener noreferrer" className="text-base md:text-lg px-3 py-1 rounded-full font-medium transition-all duration-150 text-[#222c38] hover:bg-[#f7d7db] hover:text-[#c92637]" style={{
          fontFamily: "Montserrat, Inter, sans-serif"
        }}>
            {blogMenu.label}
          </a>
